...
func NewSettingsR2Resolver(db *sql.DB, svc *settings.Service, accessKeyPath, secretKeyPath string) R2Resolver {
	return func(ctx context.Context) (*R2Delivery, error) {
		...
		access, err := ReadOptionalSecretFile(accessKeyPath)
		if err != nil {
			return nil, err
		}
		secret, err := ReadOptionalSecretFile(secretKeyPath)
		if err != nil {
			return nil, err
		}
...
// R2CredentialsPresent reports whether both R2 credential secret files are
// readable and non-empty (presence only — never the values).
