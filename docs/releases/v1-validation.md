...
- T30 交付：新增 `check-secrets.sh`、`check-security.sh`、严格 P95 计算的 `bench-search.sh`、20 成员规模与 RSS 采样的 `bench-scale.sh`、完整选择 `TestRestore*` 与归档安全校验的 `restore-drill.sh`、三视口键盘/控件验收 `accessibility.spec.ts` 和 Compose Secret 文件门禁；新增发布清单。
- T30 验证：
  ...
  7. `bash scripts/bench-scale.sh` → **通过**：20 个 `member` 账号（另有 1 个管理员）、10,000 条加密个人登录记录，每个成员 500 条，分布、字段完整性和一条 AAD 解密断言通过；Ubuntu Linux arm64（aarch64）、Go 1.26.8、4 vCPU、主机内存 23 GiB、文件系统 49G/可用 35G。加载后 GC 的 Go `HeapAlloc` 0.56 MiB、`HeapSys` 135.53 MiB，测试进程 `VmRSS` 25,968 KiB（约 25.4 MiB）、`VmHWM` 149,784 KiB（约 146.3 MiB，含初始化管理员的单次 Argon2 峰值）；脚本采样到非零 RSS/HWM 峰值（150,164/149,788 KiB）。SQLite 主库 3,223,552 bytes、WAL 4,194,192 bytes。该测试进程工作集不能替代生产服务空闲 RSS 测量，约 100 MiB 空闲内存目标仍需独立测量与处置；未以降低 Argon2id 参数换取数值。
  8. `go test ./internal/auth -run '^$' -bench 'BenchmarkPassword(Hash|Verify)$' -benchmem -count=3` → **完成 arm64 测量但未达目标**：Neoverse-N1、4 vCPU、23 GiB 主机内存、Argon2id `m=65536,t=3,p=2`；Hash 为 113.9–116.9 ms/op，Verify 为 112.9–117.5 ms/op，约 64 MiB/op。250–500 ms 目标仍需双架构测量和参数决策。
  9. `TP_SEARCH_REPEATS=1 TP_SEARCH_P95_TARGET_MS=300 bash scripts/bench-search.sh` → **通过**：本轮全局 P95 228.172ms，最慢分组 P95 228.172ms；5 个关键词、冷/热样本均低于 300ms。此前 5 次进程重复的 227.798ms/231.476ms 结果仍作为稳定性证据。
...
- 未解决发布阻断项：真实 R2 `TestR2Live` 和 R2→新密钥恢复、Android 真机 PWA、真实 LAN HTTPS/Tunnel、双架构候选镜像 smoke/SBOM/漏洞扫描、Argon2id 双架构 250–500ms 目标、约 100 MiB 空闲内存目标及其处置。20 成员/10,000 条规模基线已完成；搜索 P95 300ms 阻断项已关闭；M6 暂不判定为最终发布完成。
