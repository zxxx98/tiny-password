...
- 当前服务端支持 `login`、`ssh_key`、`credit_card`、`identity`、`secure_note`、`secret` 六种条目；Android 列表、搜索和详情支持所有可读取类型，新增仍固定为 `login`。
- 列表和搜索省略客户端范围过滤，使用服务端默认的可读集合（当前用户个人条目及有权读取的共享条目）；新增固定传 `vault_scope: personal, item_type: login`。
- Android 可查看可读取共享条目的完整详情，但共享条目只读，修改和移入回收站通过 Web 使用。个人条目详情继续校验 `owner_id`。
...
2. Type / Scope：Inter 或 JetBrains Mono，显示服务端返回的条目类型和 `PERSONAL` / `SHARED`
...
- 顶部常驻搜索框。
- 输入后防抖约 300ms，调用 `POST /api/v1/items/search`，请求体携带 `query`，使用服务端返回的全部可读条目，不只过滤当前已加载数据。
- 匹配名称、用户名、全部 URLs、标签和备注，不搜索密码明文。标签虽然不可编辑，仍可能使既有条目命中搜索。
- 首次列表调用 `GET /api/v1/items`；列表和搜索都根据 `next_cursor` 提供 `LOAD MORE`，搜索续页在请求体携带 `cursor`。
...
V1 只编辑当前用户的个人条目。共享条目所有成员可读，但 Android 当前对共享条目统一展示只读详情，隐藏 `EDIT` 和删除操作；共享条目的修改和删除仍通过 Web 遵循服务端写权限。
