...
- 登录页提供可折叠的 `SERVER` 地址控件，首次无地址时展开。地址为 HTTPS 服务源地址，可由构建配置预填，也可在登录前修改；仅保存地址这类非敏感配置。服务端须已通过 Web 完成初始化，MVP 不提供 setup 页面。
...
- MVP 会话 Cookie 只在进程内保存，冷启动重新登录。明文密码、条目详情、草稿和 CSRF token 不写入普通本地持久化存储。
- Vault 顶栏提供 `SIGN OUT`。退出时调用 `POST /api/v1/auth/logout`，清理 Cookie、CSRF、敏感内存并取消在途请求；网络失败仍完成本地退出，同时明确提示服务端会话未确认撤销。`401` 同样清理并回到登录阶段，`403 ACCOUNT_DISABLED` 展示账号停用原因后退出。
...
- [ ] 登录页不存在生物识别按钮。
- [ ] V1 不启用生物识别或持久化会话；冷启动、退出和会话失效进入普通登录。
