...
- [ ] 在明确 CPU/内存/磁盘和架构环境中，测 20 成员、10,000 条总量；另测单用户 10,000 条搜索 P95 ≤300 ms，记录冷热缓存、关键词、样本数与并发。（搜索子项已于 2026-09-07 在 arm64/Go 1.26.8 完成：批次 200→5000 后，5 次进程重复、5 个关键词、冷热样本全局 P95 227.798ms，最慢分组 P95 231.476ms；SQL/解密/JSON/匹配阶段 profile 见 `docs/decisions/0003-search-performance.md`。20 成员/总量与内存基线仍待测）
