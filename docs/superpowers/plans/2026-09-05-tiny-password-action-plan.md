...
- [x] 在 SQL 中先限制授权范围，再分批解密、筛选和分页；按 updated_at 加 ID 稳定倒序，不落地明文索引。（scanMatches 当前 5,000 条/批；DecryptHook 计数断言解密恰等于授权候选集；性能决策见 `docs/decisions/0003-search-performance.md`）
...
- [x] 生成单用户 10,000 条合成记录，建立搜索性能基线和授权前解密调用计数断言。（fixtures 种子直插；优化前基线 2.90s、解密恰 10000 次；优化后矩阵 P95 见 T30/0003）
...
- [x] 在明确 CPU/内存/磁盘和架构环境中，测 20 成员、10,000 条总量；另测单用户 10,000 条搜索 P95 ≤300 ms，记录冷热缓存、关键词、样本数与并发。（2026-09-08 在 Ubuntu arm64/Go 1.26.8/4 vCPU 完成：20 个 member + 1 个管理员、10,000 条总量、每成员 500 条；主机内存 23 GiB、文件系统 49G/可用 35G，加载后 GC 的进程 VmRSS 25,968 KiB，VmHWM 149,784 KiB，后者含初始化管理员的单次 Argon2 峰值。单用户搜索单重复矩阵全局 P95 228.172ms；5 次进程重复稳定性结果为 227.798ms，最慢分组 231.476ms。搜索 profile 见 `docs/decisions/0003-search-performance.md`，可重复入口为 `scripts/bench-scale.sh` 与 `scripts/bench-search.sh`；生产服务空闲 RSS 与 Argon2 双架构目标仍待独立验收。）
...
go test -race ./...
make test-e2e
TP_SECRET_SCAN_PATHS="$TP_DATA_DIR:$TP_WORK_DIR:$TP_LOG_DIR:$TP_AUDIT_DIR" bash scripts/check-secrets.sh
