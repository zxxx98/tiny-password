...
V1 范围：登录（含同页强制改密）、个人与可读取共享条目的列表/搜索/分页、个人条目的查看/编辑/新增/移入回收站、共享条目的只读查看、密码生成器 Bottom Sheet。共享条目的修改和移入回收站仍需通过 Web 使用。不含生物识别、离线保险库、Settings、注册、回收站页面。
...
- 共享条目会出现在列表和搜索结果，并可在 Android 查看完整详情；共享条目保持只读，修改或移入回收站需通过 Web 使用。
