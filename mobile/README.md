...
V1 范围：登录（含同页强制改密）、个人与可读取共享条目的列表/搜索/分页、个人条目的查看/编辑/新增/移入回收站、密码生成器 Bottom Sheet。共享条目在详情页仍提示通过 Web 使用。不含生物识别、离线保险库、Settings、注册、回收站页面。
...
- 共享条目会出现在列表和搜索结果，但 Android 当前仅支持个人条目的详情与编辑；打开共享条目会提示通过 Web 使用。
